...
const { TeenagerBook, validateBook } = require("../models/bookModel");
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "books/teenagerBooks/"); // Set the destination folder where uploaded files will be stored
  },
...
router.get("/books", async (req, res) => {
  try {
    const books = await TeenagerBook.find();
...
router.get("/books/:id", async (req, res) => {
  try {
    ...
    const book = await TeenagerBook.findById(id);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const newBook = new TeenagerBook({
        title,
        description,
        category,
      });
...
router.put("/books/:id", auth(), upload.single("image"), async (req, res) => {
  ...
  try {
    const book = await TeenagerBook.findById(id).exec();
...
router.delete("/books/:id", auth(), async (req, res) => {
  ...
  try {
    ...
    await TeenagerBook.findByIdAndDelete(id).exec();
