...
const {ChildBook, validateBook } = require("../models/bookModel");
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "books/childBooks/"); // Set the destination folder where uploaded files will be stored
  },
...
router.get("/books", async (req, res) => {
  try {
    const books = await ChildBook.find();
...
router.get("/books/:id", async (req, res) => {
  try {
    ...
    const book = await ChildBook.findById(id);
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const book = await ChildBook.findById(id).exec();
...
router.delete("/books/:id", auth(), async (req, res) => {
  ...
  try {
    ...
    await ChildBook.findByIdAndDelete(id).exec();
